...
function buildCcRequest(openaiReq) {
  ...
  if (reasoning_effort !== undefined) {
    const REASONING_VALUES = ["low", "medium", "high", "xhigh", "max"];
    if (REASONING_VALUES.includes(reasoning_effort)) {
...
async function handleUsage(req, res) {
  // 透传 CC 用量 API：/billing/credits + /usage/summary
  try {
    const apiKey = getApiKey(req.headers);
    if (!apiKey) { sendJSON(res, 401, { error: { message: 'Missing API key', type: 'auth_error' } }); return; }
    const authH = { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' };
    const [credits, usage] = await Promise.all([
      fetch(`${CFG.apiBase}/alpha/billing/credits`, { headers: authH }).then(r => r.ok ? r.json() : null),
      fetch(`${CFG.apiBase}/alpha/usage/summary`, { headers: authH }).then(r => r.json().catch(() => null)),
    ]);
    sendJSON(res, 200, { credits, usage });
  } catch (e) {
